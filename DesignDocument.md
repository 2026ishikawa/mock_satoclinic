...
予約データの重複判定とデータ品質を安定させるため、`reservations` テーブルへ保存する際は以下の正規化を行う。

### 12.1 氏名 `patient_name`
